...
node --experimental-strip-types skills/skill-cleaner/scripts/skill-cleaner.ts --root /path/to/archive/skills --no-logs
...
- It reads the configured root model from `~/.codex/config.toml`, then looks up that model in `~/.codex/models_cache.json`; fallback is 272,000 tokens and 2%.
- It scans normal Codex, plugin, `~/.agents`, and repo skill roots by default. Extra archive folders are included only with `--root <path>`.

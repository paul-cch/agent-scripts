...
skills/release-mac-app/scripts/mac-release codesign-run [--with-package-secrets] -- <command> [args...]
```
...
- `MAC_RELEASE_OP_ACCOUNT` is optional; set it only for the desired 1Password account. `MAC_RELEASE_OP_VAULT`, `MAC_RELEASE_OP_TMUX_SESSION`, and `MAC_RELEASE_OP_WAIT_SECONDS` are optional. Without a vault, service-account token env is unset for that single `op` read so the personal desktop account handles it.
